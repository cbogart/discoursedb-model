...
**STEP 7:**: A DiscourseRelation between two Contributions can be created using ```discourseRelationService.createDiscourseRelation(Contribution sourceContribution, Contribution targetContribution, DiscourseRelationTypes type)```. In our forum example, a Contribution _reply_ that replies to a Cotribution _parent_ would be linked by ```discourseRelationService.createDiscourseRelation(parent, reply, DiscourseRelationTypes,REPLY)```.
